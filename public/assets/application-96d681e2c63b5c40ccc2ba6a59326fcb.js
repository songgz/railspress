  ...
  ready = function() {
    ...
    if ($('.stick_div').length) {
      if ($('.page_main').height() > $('.sidebar').outerHeight(true)) {
        ...
        return $('.stick_div').affix({
          offset: {
            top: affix_start,
            bottom: function() {
              return this.bottom = $('.page_footer').outerHeight(true) + 30;
            }
          }
        });
      }
...
(function() {
  ...
  window.MyAdSense = new AdSense("ca-pub-5366654401495221");
...
(function() {
  var getCaretPosition, insertTextAtPosision, random_str, ready;
  ...
  ready = function() {
    if ($('.image_widget').length) {
      ...
      $('.image_widget').fileupload('option', {
        ...
        imageCrop: false,
        add: function(e, data) {
          var display_size, file, rand_id, types;
          ...
          if (types.test(file.type) || types.test(file.name)) {
            if (file.size < 5000000) {
              ...
              return alert("File size is too big. Maximum:5MB (was " + display_size + "MB) ");
            }
          } else {
        ...
        progress: function(e, data) {
          ...
          if (data.context) {
            ...
            return data.context.find('.progress-bar').css('width', progress + '%').attr('aria-valuenow', progress);
          }
        }
      });
    }
    if ($('.image_widget').length) {
      $('#images').on('click', '.thumb', function(event) {
        var alt, caret_pos, tag, url;
        if (!$(this).hasClass('wating')) {
          caret_pos = getCaretPosition($('.post_content').get(0));
          url = $('img', this).data('image-url').replace('https', 'http').replace('workabroad.s3.amazonaws.com', 'image.workabroad.jp');
          alt = $('img', this).attr('alt');
          tag = "![" + alt + "](" + url + ")";
          return insertTextAtPosision($('.post_content').get(0), caret_pos, tag);
        }
      });
    }
    return $('.my_folder').on('click', function(event) {
      return $(this).addClass('disabled');
    });
  };

  random_str = function(num) {
    var i, len, letters, rand, str, _i;
    str = "";
  ...
  insertTextAtPosision = function(obj, pos, txt) {
    ...
    return obj.setSelectionRange(np, np);
  };

  getCaretPosition = function(obj) {
    var pos;
    obj.focus();
    return pos = obj.selectionStart;
  };
